...
export function RuntimeSettingsPicker({
  ...
  defaultProfileLabel = "Workspace default",
  profileDisabled = false,
  codexSettingsDisabled = false,
  agentLocked = false,
  disabled,
}: {
  agents: AgentCatalogEntry[];
        ...
        >
          ...
          {section && (
            <div className="runtime-settings-submenu" ref={submenuRef} role="menu" aria-label={`${section} options`}>
              ...
              {section === "profile" && allowDefaultProfile && (
                <button
                  type="button"
                  role="menuitemradio"
                ...
                >
                  <span>{defaultProfileLabel}</span>
                  {selectedProfile === "" && <Check size={15} strokeWidth={2} />}
                </button>
              )}
