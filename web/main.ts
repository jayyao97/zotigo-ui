...
initializePreferencesStore(process.env.ZOTIGO_WEB_DATA_DIR ?? path.join(os.homedir(), ".zotigo", "web"));
const server = createWebServer({ origin, token, assetsPath: path.resolve(__dirname, "../../dist") });
server.listen(port, host, () => {
  console.log(`Zotigo Web: ${origin}`);
  if (!process.env.ZOTIGO_WEB_TOKEN) console.log(`Access token (valid until restart): ${token}`);
});
