...
import { listChannelConnections, listChannelConversations, setDaemonBaseUrl, UnsupportedChannelsError, updateChannelConversation, ZotigodRequestError } from "../backend/zotigod";

let server: http.Server;
let mode: "valid" | "malformed" | "missing" = "valid";
...
test("a missing Channel resource remains a normal not-found error", async () => {
  ...
  await assert.rejects(() => updateChannelConversation("missing", {
    display_name: "", session_id: "", workspace_id: "", enabled: false, allowed_sender_ids: [],
...
function conversation(): Record<string, unknown> {
  return {
    id: "conversation-1", connection_id: "connection-1", chat_id: "chat-1", chat_type: "group",
    display_name: "Topic", enabled: true, allowed_sender_ids: ["owner-1"], observed_senders: null,
